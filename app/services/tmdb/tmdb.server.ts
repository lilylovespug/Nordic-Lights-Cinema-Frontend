...
export const getMovieDetail = async (
  ...
  try {
    const movieData = await getMovieDetailAsync(id.toString());
    var details: IMovieDetail = {
      title: movieData.name,
      poster_path: movieData.cover,
...
const getMovieDetailAsync = async (movieId: string) => {
  const loadMoviesUrl = `http://localhost:5000/api/movies/${movieId}`;
  const movie: MovieData = await (await fetch(loadMoviesUrl)).json();

  return movie;
};

export const loadMoviesAsync = async (request: Request) => {
  console.log('start loadMoviesAsync');
  if (!request) {
    console.log('request is null');
    return defaultMovies;
  }
  const winUrl = new URL(request.url);
  const country = winUrl.searchParams.get('country');
  const cinemaId = winUrl.searchParams.get('cinemaId');

  const loadShowsUrl = `http://localhost:5000/api/cinemas/${country}/${cinemaId}/shows`;
  console.log({ url: loadShowsUrl });
  const response = await fetch(loadShowsUrl, {
    method: 'GET',
    headers: {
  ...
  const fullShows: FullShowData[] = [];
  for (let show of shows) {
    const movieId = show.movieId;
    const movie = await getMovieDetailAsync(movieId);

    const fullShow: FullShowData = {
      id: show.id,
    ...
    fullShows.push(fullShow);
  }

  console.log({ fullShows });

  const items: IMedia[] = fullShows.map((show) => {
    return {
      id: `${country}|${show.cinemaId}|${show.movieId}|${show.id}`,
      title: show.title,
      posterPath: show.cover,
      mediaType: 'movie',
      name: show.title,
...
const defaultMovies: IMediaList = {
  page: 1,
  totalPages: 1,
  totalResults: 100,
  items: [
    { id: 1, title: 'Movie 1', posterPath: '/poster1.jpg', mediaType: 'movie' },
    { id: 2, title: 'Movie 2', posterPath: '/poster1.jpg', mediaType: 'movie' },
  ],
};
...
export const getListDiscover = async (
  ...
  return await loadMoviesAsync(request);
};
